...
function FitsPage() {
  ...
  const tSeriesData = useMemo(() => {
    ...
    return data.bins.map((bin: string) => {
      ...
      filters.filter(f => f !== "All Models").forEach(model => {
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="glass-card fade-in-up delay-75">
          ...
          <CardContent className="h-[250px] w-full p-2 pl-0">
            <ResponsiveContainer>
              <LineChart data={tSeriesData} margin={{ left: -20, right: 10, top: 10, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="Jüttner 1c" stroke="var(--rose-brand)" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="Tsallis 2c" stroke="var(--emerald-brand)" strokeWidth={2} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="Bose-Einstein 1c" stroke="var(--primary)" strokeWidth={2} dot={{ r: 3 }} />
                {data?.compareSeries && (
                  <>
                    <Line
                      type="monotone"
      ...
      <div className="mt-4 space-y-3">
        {dataAlerts.corrAlerts.length > 0 && (
          <Alert className="glass-card border-amber-brand/40">
            ...
            <AlertTitle>
              {dataAlerts.corrAlerts.length} high-correlation pair{dataAlerts.corrAlerts.length > 1 ? "s" : ""} detected
            </AlertTitle>
            <AlertDescription className="mt-1 space-y-0.5 text-xs font-mono">
              ...
              {dataAlerts.corrAlerts.length > 5 && (
                <div className="text-muted-foreground">…and {dataAlerts.corrAlerts.length - 5} more</div>
              )}
            </AlertDescription>
          </Alert>
        )}
        {dataAlerts.chi2Alerts.length > 0 && (
          <Alert className="glass-card border-rose-brand/40">
            ...
            <AlertTitle>
              {dataAlerts.chi2Alerts.length} fit{dataAlerts.chi2Alerts.length > 1 ? "s" : ""} exceed χ²/ndf = 10
            </AlertTitle>
            <AlertDescription className="mt-1 space-y-0.5 text-xs font-mono">
              ...
              {dataAlerts.chi2Alerts.length > 5 && (
                <div className="text-muted-foreground">…and {dataAlerts.chi2Alerts.length - 5} more</div>
              )}
            </AlertDescription>
          </Alert>
        )}
...
function FitDetailSheet({ row, runId, onClose }: { row: FitRow | null; runId?: string; onClose: () => void }) {
  ...
  const covMatrix = useMemo(() => {
    ...
    return params.map(p1 =>
      params.map(p2 => row.correlations[`${p1}|${p2}`] ?? row.correlations[`${p2}|${p1}`] ?? (p1 === p2 ? 1.0 : 0))
    );
            ...
            <div className="mt-6 space-y-6 px-1">
              ...
              <section>
                ...
                >
                  {covMatrix.flat().map((v, i) => {
                    ...
                    const isHigh = Math.abs(v) > 0.9 && i % params.length !== Math.floor(i / params.length);
                    return (
                      <div
                        key={i}
              ...
              <section>
                ...
                <div className="rounded-md border border-border bg-muted/20 p-4 text-sm text-muted-foreground text-center italic">
                  Residual plots require per-point fit data. Re-run fits with <code>--save-residuals</code> flag to enable this panel.
                </div>
              </section>
