...
async def run_research_task(ctx: dict[str, Any], *, run_id: str, query: str, config: dict | None = None) -> dict:
    ...
    try:
        ...
        # Build the graph/agent. We use the run_id as the thread_id.
        agent = make_lead_agent(RunnableConfig(configurable={"thread_id": run_id}, **(config or {})))
        
        # Dispatch to the lead agent graph
        final_state = await agent.ainvoke({"messages": [HumanMessage(content=query)]})
        
        # Extract the final report from the last message in the thread
        messages = final_state.get("messages", [])
        report_text = messages[-1].content if messages else "[No report generated]"
