...
class MindIEChatModel(ChatOpenAI):
    ...
    def __init__(self, **kwargs):
        ...
        read_timeout = kwargs.pop("read_timeout", 300.0)
        write_timeout = kwargs.pop("write_timeout", 60.0)
        pool_timeout = kwargs.pop("pool_timeout", 300.0)

        explicit_timeout = kwargs.get("timeout", None)
        if isinstance(explicit_timeout, (int, float)):
            read_timeout = float(explicit_timeout)
            pool_timeout = float(explicit_timeout)

        kwargs.setdefault("max_retries", kwargs.pop("max_retries", 3))
