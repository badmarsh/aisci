...
# ---------------------------------------------------------------------------
# 43. MindIEChatModel prompt injection
# ---------------------------------------------------------------------------
@patch("deerflow.agents.factory.create_agent")
def test_mindie_chat_model_injects_xml_prompt(mock_create_agent):
    mock_create_agent.return_value = MagicMock()
    
    class MockMindIEModel(MagicMock):
        pass
    
    model = MockMindIEModel()
    model.__class__.__name__ = "MindIEChatModel"
    
    create_deerflow_agent(
        model,
        system_prompt="Original prompt.",
        tools=[_make_mock_tool("some_tool")],
        features=RuntimeFeatures(sandbox=False),
    )
    
    call_kwargs = mock_create_agent.call_args[1]
    injected_prompt = call_kwargs["system_prompt"]
    assert "Original prompt." in injected_prompt
    assert "**TOOL USE INSTRUCTIONS:**" in injected_prompt
    assert "<tool_call>" in injected_prompt
