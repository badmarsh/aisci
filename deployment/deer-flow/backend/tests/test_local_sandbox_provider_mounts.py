...
class TestLocalSandboxProviderMounts:
    ...
    def test_setup_path_mappings_rejects_sensitive_host_paths(self, tmp_path):
        skills_dir = tmp_path / "skills"
        skills_dir.mkdir()
        ...
        sandbox_config = SandboxConfig(
            use="deerflow.sandbox.local:LocalSandboxProvider",
            mounts=[
                VolumeMountConfig(host_path="/var/run/docker.sock", container_path="/mnt/docker", read_only=False),
                VolumeMountConfig(host_path=str(Path.home() / ".ssh" / "id_rsa"), container_path="/mnt/ssh", read_only=True),
            ],
        )
        config = SimpleNamespace(
            skills=SimpleNamespace(container_path="/mnt/skills", get_skills_path=lambda: skills_dir),
        ...
        # Both sensitive mounts should be skipped, leaving only the default skills mount
        assert [m.container_path for m in provider._path_mappings] == ["/mnt/skills"]
