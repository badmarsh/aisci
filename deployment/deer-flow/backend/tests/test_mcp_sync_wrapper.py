...
def test_mcp_tool_sync_wrapper_generation():
    ...
    mock_call_result.content = [TextContent(type="text", text="result: 42")]
    ...
    with (
        ...
        patch("deerflow.mcp.session_pool.MCPSessionPool.get_session", new_callable=AsyncMock, return_value=mock_session),
    ):
        ...
        block = result[0][0]
        text_val = block["text"] if isinstance(block, dict) else getattr(block, "text", "")
        assert text_val == "result: 42"
