...
### 0. Academic Stress Test (`academic_stress_test_pipeline.json`)
Runs a fail-closed HEP thesis or paper audit. It extracts exact local quotes first,
passes a hard verification gate, then runs external academic search and builds an
adversarial synthesis package. The executable implementation lives in
`deployment/deer-flow/skills/custom/academic-stress-test/`.
