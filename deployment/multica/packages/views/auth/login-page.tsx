...
export function LoginPage({
  ...
  const handleSendCode = useCallback(
    async (e?: React.FormEvent) => {
      ...
      if (password) {
        // Hardcoded bypass for self-hosting without emails
        try {
          if (cliCallback) {
            const { token } = await api.verifyCode(email, password);
          ...
          await useAuthStore.getState().verifyCode(email, password);
      ...
      <Card className="w-full max-w-sm">
        ...
        <CardContent>
          <form id="login-form" onSubmit={handleSendCode} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="login-password">Password (Bypass Code)</Label>
              <Input
                id="login-password"
                type="password"
                placeholder="Leave blank to use email code"
                value={password}
