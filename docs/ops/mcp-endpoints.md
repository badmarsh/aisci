...
| Onyx MCP | Local SSE | `http://127.0.0.1:8095/onyx/sse` | Server-side ONYX_API_KEY injected into `onyx_mcp_server` | ✅ Search active; chat route fixed but LLM answer generation blocked | 2026-05-02: `search_onyx` verified over SSE against Onyx docs (`Onyx MCP Server - Onyx Documentation`). `chat_with_onyx` now reaches `/chat/send-chat-message`, but current persona/model generation returns empty answer with LiteLLM/OpenRouter credit or max-token errors. DeerFlow uses `http://host.docker.internal:8095/onyx/sse` |
...
| Serena | Stdio MCP | Host: `serena start-mcp-server --context=codex --project=/home/ubuntu/aisci`; DeerFlow: Docker-backed stdio mount `/home/ubuntu/aisci:/workspaces/aisci` | None | ✅ Installed; stdio launch verified | 2026-05-03: host launch listed 20 Serena tools; Docker-backed launch listed 22 tools. `deer-flow-gateway` can read the configured entry and execute the Docker image through the mounted Docker socket. Restart or wait for MCP cache mtime reload if a live DeerFlow session does not show Serena immediately. |
