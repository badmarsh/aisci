...
|---|---|---|---|
| Fix Transformer Bug: Patch transformer.py to handle missing/nested KeyError: 'document_id' in Vespa chunks. | Phase 2 | Done | Was blocking OpenSearch migration |
...
| Audit Ollama model list: run `ollama list` and resolve the "no models found" vs. "contextual RAG working" contradiction. | Phase 2 | High | Ollama |
...
| Configure Scite + Consensus API keys in `nginx_mcp_proxy.conf` and run a test query for each | Phase 3 | High | Scite/Consensus MCP |
