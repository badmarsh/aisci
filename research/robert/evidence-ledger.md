...
| Jüttner 3-component parameterization is mathematically singular at U → 0 | Symbolic construction of Fisher Information matrix and algebraically showing the determinant vanishes | **2026-07-12 Singularity Analysis**: Symbolic derivation (`runs/2026-07-12-d01-juttner-singularity-analysis/`) shows the derivative of the integrand with respect to U evaluates to an odd function of pseudorapidity η. Symmetrically integrating over η yields exactly zero at U=0, causing a column of zeros in the Jacobian and forcing $\det(I)=0$. Proposed baseline: Two-Component Soft/Hard Model (e.g., Tsallis 2-component). | Sanity checked | Verify fit quality and correlation matrix of the proposed 2-component baseline across multiplicity bins |
| Two-Component Soft/Hard Model resolves Jüttner chi2/ndf regressions | Run of Two-Component Soft/Hard Model (Jüttner + Powerlaw tail) against 10 multiplicity bins | **2026-07-13 Jüttner-PowerLaw Run**: Implementing a true power-law tail (hard QCD scattering) on top of the exact Jüttner integration successfully resolves the massive $\chi^2/\text{ndf}$ anomalies seen in the 1c thermal models. For example, $\chi^2/\text{ndf}$ dropped from 178 to 1.59 in the highest multiplicity bin (101-125), and remains < 3 across all bins. This confirms the hypothesis that the Jüttner formulation is physically valid only for the soft component, and the power-law tail captures the high-$p_T$ deviations. | Validated | N/A |
---
...
**Next gate**: Create GitHub Issue for adding the Jacobian to the manuscript. Robert to approve.
...
|-------|-------|------|---------|-------|-----------|
| Schnedermann, Sollfrank, Heinz (SSH) — BGBW original | \cite{Schnedermann:1993ws} nucl-th/9307020 | 1993 | Phys.Rev.C 48:2462 | 1369 | Canonical BGBW model used in all blast-wave baselines |
| Cleymans, Worku — Tsallis in pp at LHC | \cite{Cleymans:2011in} 1110.5526 | 2012 | J.Phys.G 39:025006 | 235 | Closest Tsallis baseline; introduces thermodynamic consistency; STAR, PHENIX, ALICE, CMS data |
| Khuntia, Sharma, Tiwari, Sahoo — Radial flow pp | \cite{Khuntia:2018znt} 1808.02383 | 2019 | Eur.Phys.J.A (DOI: 10.1140/epja/i2019-12669-6) | 47 | BGBW vs multiplicity, pp √s=7 TeV, pions/kaons/protons; T_kin and β trends |
| Rath, Sahoo — Freeze-out vs multiplicity pp | \cite{Rath:2019cpe} 1908.04208 | 2020 | J.Phys.G (DOI: 10.1088/1361-6471/ab783b) | 31 | T_kin, β vs multiplicity pp/pA/AA; directly comparable parameter ranges |
| Bylinkin, Rostovtsev — Two-component spectra | \cite{Bylinkin:2014vra} 1407.4087 | 2014 | (Eur.Phys.J.) | 22 | Thermal + power law decomposition — alternative to 2c Jüttner |
| Parvan — Relativistic Tsallis transformations | \cite{Parvan:2024ikl} 2406.12029 | 2025 | Eur.Phys.J.A | 0 | Boltzmann-Gibbs vs Tsallis Lorentz transform properties — theory context |
| Lao, Liu, Lacey — Improved Tsallis distribution | \cite{Lao:2016gxv} 1611.08391v4 | 2016 | Eur.Phys.J.A 53:44 | - | Uses a Taylor expansion up to first order in (q-1) to approximate Tsallis distribution with radial flow |
| Lu et al. — Quantification of pion excess | \cite{Lu:2024shm} 2407.09207v3 | 2024 | NUCL SCI TECH 36 | - | Uses Bayesian inference to quantify model-to-data differences in heavy-ion collisions |
| Bendavid et al. — Angular Coefficients | \cite{Bendavid:2025urn} 2508.00989v3 | 2025 | - | - | Uses Symbolic Regression to derive analytical expressions for kinematic observables |
...
- **Finding**: Applying the unfolding matrix shifts the kinetic freeze-out temperature $T_{kin}$ by up to 4.7 MeV. While not massive, this confirms that estimator bias (SPD vs V0M) propagates directly into the thermodynamic parameter extraction. Delta table documented in `verification_report.md` (e.g. Bin 101-125 shifts from 0.0973 GeV to 0.0925 GeV).
...
## 2026-07-11 Exact Bose-Einstein Stability Check — CRITICAL FINDING

**Run dirs:**
- `research/robert/runs/2026-07-11-exact-be-grid-scan/`
- `research/robert/runs/2026-07-11-exact-be-fit-range-scan/`

**Hypothesis**: The 2-component exact Bose-Einstein model is mathematically stable across all multiplicity classes and doesn't get stuck in local minima (unlike the Jüttner approximation which failed to converge).

**Findings**:
1. **Grid Scan Convergence Failure**: Exhaustive dense grid scan over initial values ($T_1$, $U_1$, $T_2$, $U_2$) on high-multiplicity bin (126-150) returned `MIGRAD failed` (success=False) for every single configuration. The exact BE integration fails to find a stable minimum.
2. **Fit Range Instability**: Scanning the low-$p_T$ cutoff from 0.15 GeV up to 0.50 GeV on bin 21-30 also resulted in complete MIGRAD failure at every single cutoff step.

**Conclusion**: 🔴 **The 2-component exact Bose-Einstein model is NOT mathematically stable.** Like the Jüttner approximation before it, introducing a second component with flow velocity creates a heavily degenerate parameter landscape that breaks the Migrad minimizer. 
**Next Steps**: Stop using 2-component models with separate flow velocities. The model requires either (a) fixing the two flow velocities to be equal ($U_1 = U_2$), (b) using Bayesian MCMC to handle the multimodal posterior, or (c) abandoning thermal models for hard QCD tails entirely (Bíró's approach).
